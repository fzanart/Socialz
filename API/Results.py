...
class SuccessRate:
    def read(self, path):
        with open(path, encoding="utf-8") as f:
            ...
            log_file = [
                {
                    "time": re.search(r'"time": "(.*?)",', i, re.DOTALL).group(1),
                    "levelname": re.search(
                        r'"levelname": "(.*?)",', i, re.DOTALL
                    ).group(1),
                    "message": re.search(r'"message": "(.*?)"', i, re.DOTALL).group(1),
                }
                for i in lines
            ]
    ...
    def plot_df(self, df, ax, dataset_name):
        ...
        ax.plot(
            df.index,
            df["accum_CRITICAL"],
            label=f"Failed to load events {df.levelname_CRITICAL.sum()}",
        )
        ini_time = df.time[0]
        end_time = df.time[df.shape[0] - 1]
        loading_time = round(
            (pd.to_datetime(end_time) - pd.to_datetime(ini_time)).total_seconds()
            / 3600,
            1,
        )
        ax.set_title(
...
class PrometheusResuts:
    ...
    def read(self, logfile_path):
        ...
        data = pd.DataFrame(log_file).rename(columns={"time": "start"})
    ...
    def select_query(self, p, metric, init, endt, step, avg):
        ...
        METRICS = {
            "cpu": 'max(max_over_time(gitlab_sli:gitlab_service_saturation:ratio{component="cpu"}[1m]))',
    ...
    def query_db(self, input_path, output_path):
        ...
        df.loc[:, "cpu"] = df.apply(
            lambda x: self.select_query(
                p, "cpu", x["start"], x["finish"], x["delta"], True
            ),
            axis=1,
        )
        df.loc[:, "mem"] = df.apply(
            lambda x: self.select_query(
                p, "memory", x["start"], x["finish"], x["delta"], True
            ),
            axis=1,
        )
        df.loc[:, "lat"] = df.apply(
            lambda x: self.select_query(
                p, "latency", x["start"], x["finish"], x["delta"], True
            ),
            axis=1,
        )
...
class ExpandDataset:
    ...
    def random(self, original_path, evolved_path, seed=42):
        ...
        users = list(
            set(
                [
                    x
                    for x in ori[0]["source"].to_list() + ori[0]["target"].to_list()
                    if x.startswith("u: ")
                ]
            )
        )
        ...
        ran_non_follow_events = pd.DataFrame(
            {
                "source": np.random.choice(users, delta_non_follow_events),
        ...
        ran_follow_events = pd.DataFrame(
            {
                "source": list(map(users.__getitem__, sources)),
                "target": list(map(users.__getitem__, targets)),
            }
        )
        ran_follow_events = ran_follow_events.sample(
            delta_follow_events, replace=True, random_state=seed
        )
    ...
    def simple(self, original_path, evolved_path, seed=42):
        ...
        ori_non_follow_events = ori[0].sample(
            n=delta_non_follow_events, replace=True, random_state=seed
        )
        # duplicate followevents
        ori_follow_events = self.read(original_path)
        ori_follow_events = ori_follow_events[0][
            ori_follow_events[0]["type"] == "FollowEvent"
        ]
        ori_follow_events = ori_follow_events.sample(
            n=delta_follow_events, replace=True, random_state=seed
        )
        # concat data
        sim = pd.concat(
